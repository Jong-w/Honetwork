...
class Worker(nn.Module):
    def __init__(self, b, c, d, k, num_actions, device):
        ...
        self.Wrnn = nn.LSTMCell(d,self.k * self.num_actions)  # changed output shape of rnn, so we can make vector that has same length with action
    ...
    def forward(self, z, goals, hidden, mask):
        ...
        #goals = torch.stack(goals).detach().sum(dim=0)  # deactivate goals
        w = self.phi(u)   # deactivate goals
...
def feudal_loss(storage, next_v_m, next_v_w, args):
    ...
    loss = (value_w_loss - loss_worker) - args.entropy_coef * entropy # remove things about manager
