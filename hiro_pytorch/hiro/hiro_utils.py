...
class ReplayBuffer():
    def __init__(self, state_dim, goal_dim, action_dim, buffer_size, batch_size):
        ...
        self.size = 0
        self.size_high = 1
        self.state = torch.zeros((buffer_size, state_dim))
        self.goal = torch.zeros((buffer_size, goal_dim))
        self.action = torch.zeros((buffer_size, action_dim))
        self.n_state = torch.zeros((buffer_size, state_dim))
...
class LowReplayBuffer(ReplayBuffer):
    def __init__(self, state_dim, goal_dim, action_dim, buffer_size, batch_size):
        ...
        self.n_goal = torch.zeros((buffer_size, goal_dim))
...
class Hierarchy5_buffer(ReplayBuffer):
    def __init__(self, state_dim, goal_dim, subgoal_dim, action_dim, buffer_size, batch_size, freq):
    ...
    def append(self, state, goal, action, n_state, reward, done, state_arr, action_arr):
        ...
        self.ptr = (self.ptr+1) % self.buffer_size
        self.size_highe = min(self.size_high+1, self.buffer_size)

    def sample(self):
        ind = np.random.randint(0, self.size_high, size=self.batch_size)
...
class Hierarchy4_buffer(ReplayBuffer):
    def __init__(self, state_dim, goal_dim, subgoal_dim, action_dim, buffer_size, batch_size, freq):
...
class Hierarchy3_buffer(ReplayBuffer):
    def __init__(self, state_dim, goal_dim, subgoal_dim, action_dim, buffer_size, batch_size, freq):
...
class Hierarchy2_buffer(ReplayBuffer):
    def __init__(self, state_dim, goal_dim, subgoal_dim, action_dim, buffer_size, batch_size, freq):
